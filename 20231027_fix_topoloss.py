...
from utils.data_loading import BasicDataset, CarvanaDataset, BSE_EBSD_Dataset, BSE_EBSD_Dataset_3Channel_and_Gray
from utils.dice_score import dice_loss

import kornia
from kornia.augmentation import RandomRotation, RandomHorizontalFlip, RandomVerticalFlip

sys.path.append( "/home/appuser/data/torch/TopoLoss/" )
...
def train_model(
        ...
        sDir_grayscale: str = './data/happy-trails-grayscale-imgs-train_serial_0/',
        nFinetune_epochs: int = 3,
        alpha: float = 0.5,
):

    dir_img = Path(sDir_img)
    dir_mask = Path('./data/happy-trails-train-masks/')
    dir_grayscale = Path(sDir_grayscale)
    #_lambda = 1e-5
    
    # do this after we get set up.
    pretrain_epoch = epochs-nFinetune_epochs;
            ...
            for batch in train_loader:
                ...
                with torch.autocast(device.type if device.type != 'mps' else 'cpu', enabled=amp):
                    ...
                    if model.n_classes == 1:
                        ...
                        dL1 = alpha*criterion(masks_pred.squeeze(1), true_masks.float())
                        dL2 = (1-alpha)*dice_loss(torch.sigmoid(masks_pred).squeeze(1), true_masks.float(), multiclass=False)
                        #print( "dL1: {}, dL2: {}".format( dL1, dL2 ) )
                        loss = dL1 + dL2                        
                        if epoch > pretrain_epoch:
                            ## getTopoLoss( lh (activation) , gt )
                            ### should we be sampling from the batch? Where is batch size reverted to 1 in order to do this? Answered: it's not.
                            ## what size is masks_pred? We should probably confirm that it is batch x img_i x img_j x 1 and then iterate through
                            ## each member of the batch,
                            ## mask_pred size: 
                            ## torch.Size([8, 1, 600, 600])
                            for b in range(batch_size):
                                #with open( 'mask_pred_test.npy' , 'wb' ) as f:
                                    #np.save( f, masks_pred[b,0,:,:].cpu().detach().numpy() )
                                #with open( 'true_mask_test.npy' , 'wb' ) as f:
                                   #np.save( f, true_masks[b,:,:].cpu().detach().numpy() )
                                ## We need F.sigmoid( masks_pred ) bc masks_pred are the raw logits
                                print( masks_pred.shape , true_masks.shape, torch.sigmoid(masks_pred).max(), true_masks.max() ) 
                                dL3 = _lambda*topoloss_pytorch.getTopoLoss( (1-torch.sigmoid(masks_pred[b,0,:,:])) , (1-true_masks[b,:,:]), topo_size=150 )
    ...
    with wandb.init(config=config):
        ...
        try:
            train_model(
                ...
                sDir_grayscale=config.sDir_grayscale,
                nFinetune_epochs=config.nFinetune_epochs,
                alpha=config.alpha
            )
...
if __name__ == '__main__':
    ...
    par_dict = {
        ...
        'sDir_img' : {
            'value' : './data/happy-trails-color-imgs-train_serial_0/'
            #'values' : ['./data/happy-trails-color-imgs_serial_0/','./data/happy-trails-color-imgs_serial_1/','./data/happy-trails-color-imgs_serial_2/','./data/happy-trails-color-imgs_serial_3/','./data/happy-trails-color-imgs_serial_4/','./data/happy-trails-color-imgs_serial_5/','./data/happy-trails-color-imgs_serial_6/','./data/happy-trails-color-imgs_serial_7/','./data/happy-trails-color-imgs_serial_8/','./data/happy-trails-color-imgs_serial_9/']
        },
        'sDir_grayscale' : {
            'value' : './data/happy-trails-grayscale-imgs-train_serial_0/'
        },
        'alpha' : {
            #'distribution' : 'uniform',
            #'min' : 0,
    ...
    sweep_id = wandb.sweep(
        ...
        project='debug_topoloss'
        #project='fresh-sweeptesting'
    )

    wandb.agent(sweep_id, function=sweepfcn2, count=1)
